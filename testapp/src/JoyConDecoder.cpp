...
decode_joystick(const std::vector<uint8_t> &buffer, bool isLeft, bool upright) {
  auto res = DecodeJoystick(
      buffer, isLeft ? JoyConSide::Left : JoyConSide::Right,
      upright ? JoyConOrientation::Upright : JoyConOrientation::Sideways);
  return {res.x, res.y};
}

std::pair<uint16_t, uint16_t>
DecodeMouseCoords(const std::vector<uint8_t> &buffer) {
  if (buffer.size() < 0x18)
    return {960, 471};
...
void EncodeDS4Touch(DS4_TOUCH &touch, uint8_t trackingId, uint16_t x,
                    uint16_t y) {
...
DS4_REPORT_EX GenerateDS4Report(const std::vector<uint8_t> &buffer,
  ...
  uint32_t state = (buffer[btnOffset] << 16) | (buffer[btnOffset + 1] << 8) |
                   buffer[btnOffset + 2];
  ...
  decode_triggers_shoulders(state, isLeft, upright, leftTrigger, rightTrigger,
...
GenerateDualJoyConDS4Report(const std::vector<uint8_t> &leftBuffer,
  ...
  if (leftBuffer.size() >= 0x3C) {
    leftReport = GenerateDS4Report(leftBuffer, JoyConSide::Left,
                                   JoyConOrientation::Upright);
  }

  DS4_REPORT_EX rightReport{};
  if (rightBuffer.size() >= 0x3C) {
    rightReport = GenerateDS4Report(rightBuffer, JoyConSide::Right,
                                    JoyConOrientation::Upright);
  }
  ...
  report.Report.bSpecial =
      leftReport.Report.bSpecial | rightReport.Report.bSpecial;
  ...
  report.Report.sCurrentTouch.bTouchData2[1] =
      ((x2 >> 8) & 0x0F) | ((y2 & 0x0F) << 4);
  ...
  default:
    ...
    report.Report.wAccelX =
        combine_16(leftReport.Report.wAccelX, rightReport.Report.wAccelX);
    report.Report.wAccelY =
        combine_16(leftReport.Report.wAccelY, rightReport.Report.wAccelY);
    report.Report.wAccelZ =
        combine_16(leftReport.Report.wAccelZ, rightReport.Report.wAccelZ);

    report.Report.wGyroX =
        combine_16(leftReport.Report.wGyroX, rightReport.Report.wGyroX);
    report.Report.wGyroY =
        combine_16(leftReport.Report.wGyroY, rightReport.Report.wGyroY);
    report.Report.wGyroZ =
        combine_16(leftReport.Report.wGyroZ, rightReport.Report.wGyroZ);
    break;
  }
...
DS4_REPORT_EX GenerateProControllerReport(const std::vector<uint8_t> &buffer) {
  ...
  if (state & BUTTON_A_MASK)
    report.Report.wButtons |= DS4_BUTTON_CIRCLE;
  if (state & BUTTON_B_MASK)
    report.Report.wButtons |= DS4_BUTTON_CROSS;
  if (state & BUTTON_X_MASK)
    report.Report.wButtons |= DS4_BUTTON_TRIANGLE;
  if (state & BUTTON_Y_MASK)
    report.Report.wButtons |= DS4_BUTTON_SQUARE;
  if (state & BUTTON_L_SHOULDER)
    report.Report.wButtons |= DS4_BUTTON_SHOULDER_LEFT;
  if (state & BUTTON_R_SHOULDER)
    report.Report.wButtons |= DS4_BUTTON_SHOULDER_RIGHT;
  if (state & BUTTON_L_THUMB)
    report.Report.wButtons |= DS4_BUTTON_THUMB_LEFT;
  if (state & BUTTON_R_THUMB)
    report.Report.wButtons |= DS4_BUTTON_THUMB_RIGHT;
...
DS4_REPORT_EX GenerateNSOGCReport(const std::vector<uint8_t> &buffer) {
  ...
  if (state & BUTTON_R_SHOULDER)
    report.Report.wButtons |= DS4_BUTTON_SHOULDER_RIGHT;
  if (state & TRIGGER_LT_MASK)
    report.Report.wButtons |= DS4_BUTTON_TRIGGER_LEFT;
  if (state & TRIGGER_RT_MASK)
    report.Report.wButtons |= DS4_BUTTON_TRIGGER_RIGHT;
  if (state & BUTTON_L_THUMB)
    report.Report.wButtons |= DS4_BUTTON_THUMB_LEFT;
  if (state & BUTTON_R_THUMB)
